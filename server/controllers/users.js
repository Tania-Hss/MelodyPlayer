...
router.post('/users', (req, res, next)=> {
    ...
    if(!username || !email || !password) {
        const customError = new Error('Name, email and password fields are required')
    ...
    if (!/[a-z]/.test(password) || !/[A-Z]/.test(password) || !/\d/.test(password) || !/[!@#$%^&*]/.test(password)) {
        const customError = new Error('Password must contain at least one uppercase and lowecase letter, a number and one special character.')
        customError.status = 400
        return next(customError)
    }
